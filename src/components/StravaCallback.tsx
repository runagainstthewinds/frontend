...
const StravaCallback: React.FC = () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        window.location.href =
          window.location.origin + "/dashboard?refresh=true";
      }, 3000);
    } catch (error) {
  ...
  const exchangeToken = async (code: string, state: string | null) => {
    try {
      ...
      setIsSuccess(true);
      setStatus(
        `Successfully connected Strava account!${
          response.data.athleteName
            ? ` Welcome, ${response.data.athleteName}!`
            : ""
        }`,
      );
      ...
      if (axios.isAxiosError(error) && error.response) {
        setStatus(
          `Error connecting Strava: ${error.response.data.message || error.message}`,
        );
      } else {
